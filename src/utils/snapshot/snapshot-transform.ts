...
import { ErrorCode, UniverType, isError } from '@univerjs/protocol'

import type { ILogContext } from '@univerjs/core/lib/types/services/log/context'
import { type IDocumentData, type IWorkbookData, type IWorksheetData, LocaleType, SheetTypes } from '@univerjs/core'
import {
  ...
  const blocksSaveSuccess = await Promise.all(
    Object.entries(workbook.sheets).map(async ([sheetID, worksheet]) => {
      ...
      if (worksheet.cellData) {
        const sheetBlocks = splitCellDataToBlocks(worksheet.cellData as any, worksheet.rowCount!)
...
export function transformSnapshotToWorkbookData(
  ...
  if (!workbookMeta) {
    throw new Error('not found workbookMeta')
  }
